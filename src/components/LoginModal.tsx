...
const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose }) => {
  ...
  try {
    ...
    console.warn("LoginModal: useNavigate not available, navigation will be skipped");
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        onClose();
        resetForm();
        // Navigate to return path if available
        if (returnPath && navigate) {
          navigate(returnPath);
        }
      } else {
        const metadata = {
